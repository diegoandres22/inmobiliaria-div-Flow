...
function SheetOverlay({
  ...
  return (
    <SheetPrimitive.Overlay
      ...
      className={cn(
        "fixed inset-0 z-50 bg-brand-ink/40 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0",
        className,
      )}
      {...props}
    />
...
const sheetVariants = cva(
  "fixed z-50 flex flex-col gap-4 bg-background shadow-lg transition ease-in-out data-[state=closed]:duration-300 data-[state=open]:duration-500",
  {
    variants: {
      side: {
        // Mobile-first: bottom-sheet es el default para filtros — nunca un formulario largo inline
        bottom:
  ...
  return (
    <SheetPortal>
      ...
      >
        ...
        {children}
        <SheetPrimitive.Close className="absolute top-4 right-4 flex size-11 items-center justify-center rounded-full opacity-70 transition-opacity hover:opacity-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
...
function SheetHeader({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("flex flex-col gap-1.5 px-6 pt-2", className)}
      {...props}
    />
  );
}
